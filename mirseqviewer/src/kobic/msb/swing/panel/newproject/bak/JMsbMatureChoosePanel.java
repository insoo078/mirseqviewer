...
public class JMsbMatureChoosePanel extends JPanel implements Observer{
	/**
	 * 
	 */
	...
	public JMsbMatureChoosePanel( JProjectDialog owner ) {
		...
		JLabel lblSelectMirnaFor = new JLabel("Select miRNA for display");
			...
			gl_rootPanel.createParallelGroup(Alignment.TRAILING)
				.addGroup(gl_rootPanel.createSequentialGroup()
					...
					.addGroup(gl_rootPanel.createParallelGroup(Alignment.LEADING)
						...
						.addComponent(lstMirnaScrollPane, GroupLayout.DEFAULT_SIZE, 422, Short.MAX_VALUE)
						.addComponent(lblListOfMirna)
						.addGroup(gl_rootPanel.createSequentialGroup()
							.addComponent(lblSelectMirnaFor, GroupLayout.PREFERRED_SIZE, 235, GroupLayout.PREFERRED_SIZE)
...
//			e.printStackTrace();
	...
	public void updateCurrentState( String projectName ) {
		this.choosedMiRnas.clear();
		this.defaultMiRnas.clear();
		this.revalidateChoosedMiRnasTable();
		...
		try {
			JMsbNGSTestCaller worker = new JMsbNGSTestCaller( this.owner, item );
			worker.run();
		}catch(Exception e) {
			MsbEngine.logger.error( e );
		}
